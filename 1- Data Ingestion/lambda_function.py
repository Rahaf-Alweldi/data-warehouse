...
# Executing queries to copy the 'inventory.csv' rows to the Snowflake table
...
def lambda_handler(event, context):
    ...
    # 1a- load information from toml and .env files
    config = toml.load('config.toml')
    load_dotenv('.env')


    # 1b- setting the inforamtion into dictionary
    file_info = {
            'url': config['file']['url'],
            'destination_folder': config['file']['destination_folder'],
            'file_name': config['file']['file_name'],
            'local_file_path': config['file']['local_file_path'] 
        }
